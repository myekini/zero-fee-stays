...
class AnalyticsSyncService {
  ...
  private async syncMixpanelData(): Promise<void> {
    try {
      ...
      for (const eventName of events) {
        const { data: mixpanelData, error } = await supabase.functions.invoke(
          "analytics-mixpanel",
          {
            body: {
              event: eventName,
        ...
        if (error) {
          console.error(
            `Error fetching Mixpanel data for ${eventName}:`,
            error
          );
          continue;
        }
  ...
  private async cacheMetric(metric: AnalyticsMetric): Promise<void> {
    try {
      const { error } = await supabase.rpc("upsert_analytics_metric", {
        ...
        p_dimensions: metric.dimensions || undefined,
        p_entity_id: metric.entity_id || undefined,
        p_entity_type: metric.entity_type || undefined,
  ...
  startPeriodicSync(intervalMinutes: number = 60): void {
    ...
    console.log(
      `Starting periodic analytics sync every ${intervalMinutes} minutes`
    );
